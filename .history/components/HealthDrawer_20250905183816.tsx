...
const AppleHealthKit = {
  isAvailable: (callback: (error: any, results: boolean) => void) => {
    if (Platform.OS === 'ios') {
  ...
  getAuthStatus: (permissions: any, callback: (error: string, results: any) => void) => {
    // Simuler des permissions accordées
    setTimeout(() => callback(null, { steps: 2, heartRate: 2, activeEnergy: 2, distance: 2 }), 100);
  },
  initHealthKit: (permissions: any, callback: (error: string) => void) => {
    if (Platform.OS === 'ios') {
...
export default function HealthDrawer({ visible, onClose }: HealthDrawerProps) {
  ...
  const fetchHealthData = () => {
    ...
    AppleHealthKit.getDistanceWalkingRunning(options, (callbackError: string, results: HealthValue[]) => {
      if (!callbackError && results && results.length > 0) {
        setHealthData(prev => ({
          ...prev,
